...
#define OFFSET_PER_SECOND_PREPROC (22050 * (1 / 183) * (1 / 22050))
...
void Audio::set_music_pattern(int pattern) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if ((!looping || !foundNonLooping) &&
            (firstNonLooping || _audioState._musicChannel.master == -1 || shortest > timeLength)) {
            shortest = timeLength;
...
void Audio::FillMonoAudioBuffer(void *audioBuffer, size_t offset, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        int32_t sample = this->getSampleForChannel(0) + this->getSampleForChannel(1) + this->getSampleForChannel(2) +
                         this->getSampleForChannel(3);
...
float Audio::getSampleForSfx(rawSfxChannel &channel, float freqShift) {
    ...
    float const offset_per_sample = OFFSET_PER_SECOND_PREPROC / speed;
    ...
    if (loop_range > 0.f && next_offset >= sfx.loopRangeStart && channel.can_loop) {
        next_offset = fmod(next_offset - sfx.loopRangeStart, loop_range) + sfx.loopRangeStart;
    }
    /*
    int const loop_range_fixed = (sfx.loopRangeEnd - sfx.loopRangeStart) * 1000;
    if (loop_range_fixed > 0 && next_offset_fixed >= sfx.loopRangeStart * 1000 && channel.can_loop) {
        next_offset_fixed = (next_offset_fixed - sfx.loopRangeStart * 1000) % loop_range_fixed + sfx.loopRangeStart * 1000;
    }
    */
    ...
    */
    ...
    float waveform = this->getSampleForNote(channel.current_note, channel, channel.getChildChannel(),
                                            channel.prev_note.n, freqShift, false);
    if (crossfade > 0) {
        waveform *= (1.0f - crossfade);
        note dummyNote;
        waveform += crossfade *
                    this->getSampleForNote(channel.prev_note, channel, channel.getPrevChildChannel(), dummyNote,
                                           freqShift, true);
    }
...
float
Audio::getSampleForNote(noteChannel &channel, rawSfxChannel &parentChannel, rawSfxChannel *childChannel, note prev_note,
                        float freqShift, bool forceRemainder) {
    using std::max;
    ...
    // Speed must be 1—255 otherwise the SFX is invalid
    int const speed = max(1, (int) sfx.speed);
    float const offset_per_second = OFFSET_BASE / speed;
...
int16_t Audio::getSampleForChannel(int channel) {
    ...
    if (channel == _audioState._musicChannel.master && _audioState._musicChannel.pattern != -1) {
        ...
        if (_audioState._musicChannel.volume_step < 0 && _audioState._musicChannel.volume <= 0) {
            ...
            if (_audioState._sfxChannels[0].is_music) {
                _audioState._sfxChannels[0].sfxId = -1;
            }
            if (_audioState._sfxChannels[1].is_music) {
                _audioState._sfxChannels[1].sfxId = -1;
            }
            if (_audioState._sfxChannels[2].is_music) {
                _audioState._sfxChannels[2].sfxId = -1;
            }
            if (_audioState._sfxChannels[3].is_music) {
                _audioState._sfxChannels[3].sfxId = -1;
            }
            _audioState._musicChannel.pattern = -1;
    ...
    sample = (int16_t)(32768 * this->getSampleForSfx(_audioState._sfxChannels[channel]));

    // TODO: Apply hardware effects
    if (_memory->hwState.distort & (1 << channel)) {
        sample = (sample * 0x1249) >> 12;
    }
    return sample;
}
